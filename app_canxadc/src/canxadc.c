...
int main() {
    ...
    // 1. Initialize PS-XADC (Corrected for Vitis 2025.1)
    XAdcCfg = XAdcPs_LookupConfig(XADC_DEVICE_ID);
    if (XAdcCfg == NULL) return XST_FAILURE;
    
    // Note: Using BaseAddress (not BaseAddr)
    ...
    xil_printf("--- System Sync: XADC -> CAN/LCD/UART ---\r\n");
    ...
    while(1) {
        ...
        // Send to CAN (Scaled by 100, Big Endian for Waveshare)
        ...
        // Update UART
        xil_printf("Live Temp: %d.%d C\r\n", (int)Temp, (int)(Temp * 10) % 10);

        // Update LCD Lines
        lcd_send_internal(0x80, 0); // Line 1
